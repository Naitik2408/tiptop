...
function Navbar() {
    ...
    const handleConfirmOrder = () => {
        ...
        const orderDetails = addedDishes.map((dish, index) => {
            const quantityDetail = dish.quantity ? `(${dish.quantity})` : '';
            const priceDetail = `(${dish.price} x ${dish.quantity} = ${dish.price * dish.quantity})`;
            return `${index + 1}️⃣ ${dish.name} ${quantityDetail} - ${dish.quantity} ${dish.quantity > 1 ? 'Pieces' : 'Piece'} ${priceDetail}`;
        }).join('\n');
